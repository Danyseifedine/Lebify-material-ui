...
// src/buttons/assets/ts/components.ts
var components = {
  custom_1: (text) => `
    <span class="bubbly-text">${text}</span>
    `,
  custom_3: (text) => `
      <span class="lebify-btn-custom-3-folderContainer">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 146 113" height="113" width="146"
            class="lebify-btn-custom-3-fileBack">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 88 99" height="99" width="88"
            class="lebify-btn-custom-3-filePage">
            <rect fill="url(#paint0_linear_117_6)" height="99" width="88"></rect>
            <defs>
                <linearGradient gradientUnits="userSpaceOnUse" y2="160.5" x2="81" y1="0" x1="0"
        ...
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 160 79" height="79" width="160"
            class="lebify-btn-custom-3-fileFront">
      ...
      <p class="lebify-btn-custom-3-text">${text}</p>
    `,
  custom_4: (text) => `
    <svg class="svg-icon" width="24" viewBox="0 0 24 24" height="24" fill="none"><g stroke-width="2" stroke-linecap="round" stroke="#056dfa" fill-rule="evenodd" clip-rule="evenodd"><path d="m3 7h17c.5523 0 1 .44772 1 1v11c0 .5523-.4477 1-1 1h-16c-.55228 0-1-.4477-1-1z"></path><path d="m3 4.5c0-.27614.22386-.5.5-.5h6.29289c.13261 0 .25981.05268.35351.14645l2.8536 2.85355h-10z"></path></g></svg>
      ...
      `,
  custom_5: (text) => `
    <span class="lebify-btn-custom-5-spark"></span>     
    ...
    <span class="lebify-btn-custom-5-text">${text}</span>
    `,
  custom_6: (text) => `
    <div class="svg-wrapper-1">
      <div class="lebify-btn-custom-6-svg-wrapper">
  ...
  custom_12: (text) => `
    ...
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 58 44" class="box-body box">
        ...
        <rect mask="url(#path-1-inside-1_81_19)" stroke-width="8" stroke="#6A8EF6" fill="#101218" rx="3"
            height="44" width="58"></rect>
  ...
  custom_72: (text) => `
                  ...
                  <span class="text">${text}</span>
        `,
  // custom_2 is a default
  // custom_7 is a default
  // custom_8 is a default
  // custom_16 is a default 
  // custom_17 is a default
  // custom_18 is a default
...
function getComponent(type, text) {
  return components[type]?.(text) || components.default(text);
}

// src/buttons/assets/ts/animation.ts
var __Animation__ = class {
  static init_1(button) {
  ...
  static init_2(button) {
    button.addEventListener("mousemove", function(e) {
      ...
      button.style.transform = `translateX(${transX}px) translateY(${transY}px)`;
    });
...
// src/buttons/assets/ts/btn.ts
var LebifyButton = class {
  buttonElement;
  settings;
  constructor(buttonElement, options) {
    this.buttonElement = buttonElement;
    this.settings = { ...{ type: "default", text: "" }, ...options };
    this.initializeButton();
  }
  initializeButton() {
    const buttonHTML = getComponent(this.settings.type, this.buttonElement.textContent || this.settings.text);
    this.buttonElement.innerHTML = buttonHTML;
    this.initializeButtonAnimation();
  }
  initializeButtonAnimation() {
    const animationMap = {
      "custom_1": __Animation__.init_1,
      "custom_2": __Animation__.init_2
    };
    const animationFunction = animationMap[this.settings.type] || (() => {
    });
    animationFunction(this.buttonElement, this.settings);
  }
};
function handleButtonClick(event) {
  const target = event.target;
  if (target.classList.contains("lebify-btn")) {
    const type = target.dataset.type || "default";
    const text = target.dataset.text || "";
    new LebifyButton(target, { type, text });
  }
}
document.addEventListener("DOMContentLoaded", () => {
  const buttons = document.querySelectorAll(".lebify-btn");
  buttons.forEach((button) => button.addEventListener("click", handleButtonClick));
  const observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        const button = entry.target;
        const type = button.dataset.type || "default";
        const text = button.dataset.text || "";
        new LebifyButton(button, { type, text });
  ...
  for (let i = 1; i <= 72; i++) {
    buttonTypes.push({
      selector: `.lebify-btn-custom-${i}`,
      type: `custom_${i}`,
      text: specialTexts[i] || baseText
    });
  }
  buttonTypes.forEach((button) => {
    document.querySelectorAll(button.selector).forEach((element) => {
      new LebifyButton(element, { type: button.type, text: button.text });
    });
  });
});
